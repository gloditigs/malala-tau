(function ($) {
    ...
    window.slider_animation = ()=>{
        var swiper_animate_items = null;
        $(".swiper-group-animate").each(function () {
            let settings = $(this).data("settings");
            swiper_animate_items = new Swiper(this, {
                spaceBetween: settings?.space || 24,
                slidesPerView: settings?.slides_per_view || 'auto',
                slidesPerGroup: settings?.slides_per_group || 1,
                loop: settings?.slider_loop || true,
                speed: settings?.slider_speed || 1000,
                navigation: {
                    nextEl: settings?.slides_nav_next || ".swiper-button-next-animate",
                    prevEl: settings?.slides_nav_prev || ".swiper-button-prev-animate"
                },
                autoplay: {
                    delay: settings?.slider_autoplay_delay || 10000
                },
                on: {
                    afterInit: function () {
                        // set padding left slide fleet
                ...
                breakpoints: {
                    1199: {
                        slidesPerView: settings?.slides_per_view_large || "auto"
                    },
                    600: {
                        slidesPerView: settings?.slides_per_view_tab || "auto"
                    },
                    575: {
    ...
    $(window).resize(function () {
        if ($(".padding-left-auto") == null || $(".padding-right-auto") == null || $(".container") == null) {
            return;
        }
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if(shareButton){
            shareButton.addEventListener('click', function (e) {
                ...
                const link = shareButton.getAttribute('href');

                // Check if the Clipboard API is supported
                if (navigator.clipboard && navigator.clipboard.writeText) {
                    // Use Clipboard API to copy the link
                    navigator.clipboard.writeText(link).then(() => {
                        // Show an alert once the link is copied
...
//Perfect Scrollbar
if (document.querySelector(".mobile-header-wrapper-inner") !== null || document.querySelector(".sidebar-canvas-container") !== null ) {
